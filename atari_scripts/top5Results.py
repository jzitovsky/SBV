...
for i in range(num_sets):
    ...
    scale_min = wis_dict.sort_values('returns').head(5)['returns'].mean()
    scale_max = wis_dict.sort_values('returns', ascending=False).head(5)['returns'].mean()
    wis_top5[i] = wis_dict.sort_values('WIS', ascending=False).head(5)['returns'].mean()
    wis_top5[i] = (wis_top5[i] - scale_min)/(scale_max - scale_min)
    
    target_dict = pd.read_csv(dir + '/target_dict.csv', index_col=0)
    emsbe_top5[i] = target_dict.sort_values('EMSBE').head(5)['returns'].mean()
    emsbe_top5[i] = (emsbe_top5[i] - scale_min)/(scale_max - scale_min)
    
    sbv_dict = pd.read_csv(dir + '/sbv_dict.csv', index_col=0)
    sbv_top5[i] = sbv_dict.sort_values('SBV').head(5)['returns'].mean()
...
top5_dict.to_csv('top5_dict.csv')
