...
calc_SBV = function(QRes) {
  ...
  b = t(X)%*%y/m
  XVal = model.matrix(~transform(SVal)*as.factor(AVal))
  return(X%*%(solve(Amat+best_lambda*P)%*%b))
}
...
for (degree in degrees) {
  for (lambda in lambdas) {
    if (degree>3  & lambda %in% c(0.01, 1)) next
    if (degree!=3 & lambda %in% c(1e-3, 0.02)) next
    sbv[i] = mean((calc_SBV(policies[[i]])-policies[[i]]$QFun(S,A))^2)
    i = i+1
  }
}
...
calc_emsbe = function(QRes) mean((U + gamma*pmax(QRes$QFun(SN,0), QRes$QFun(SN,1)) - QRes$QFun(S,A))^2)
...
sbv = c(sbv, mean((calc_SBV(piStar)-piStar$QFun(S,A))^2))
...
#print(bellman_table)
cor(bellman_table[,-c(1)], method = 'spearman')
